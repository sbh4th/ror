...
Below you can see the average marginal predictions for reviewers vs. panelists in terms of deviations from the consensus score:
...
#| fig-cap: Estimates from deviation model
#| 
pred <- avg_predictions(m2_magnitude, variables = "job", re_formula = NULL, ndraws = 200) |>
  as.data.frame() |>
  mutate(
    dev_val = as.numeric(as.character(group)),
...
ggplot(pred, aes(x = dev_val, y = estimate, fill = job)) +
  geom_col(position = position_dodge(width = 0.08), width = 0.08, color = "white") +
  ...
  labs(x = "Deviation from consensus (given a deviation occurred)",
