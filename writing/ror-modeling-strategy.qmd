...
- The 3 assigned reviewers read the application and score it. At the
  meeting (and *before* any discussion), they they give their initial scores.
...
- After discussion the 3 assigned reviewers agree on a **consensus score**.
- All panel members (including the 3 reviewers) then submit a **final score**. Reviewers are not bound to their own consensus
...
```{r}
#| echo: false
#| label: tbl-cihr-fields
#| fig-cap: CIHR field-by-field data availability, by level

cihr_fields <- tribble(
  ~level,       ~indicator,                                                  ~availability,
  "Application", "Committee number",                                        "Extractable",
  "Application", "Application number",                                     "Extractable",
  "Application", "Funding result",                                          "Extractable",
  "Application", "Consensus score",                                         "Extractable",
  "Application", "Resubmission status",                                     "Extractable (2023 competition onward only)",
  "Application", "Keywords/domain", "Refused -- identifiability risk",
    "Application", "Number of investigators", "Refused -- identifiability risk",
    "Application", "Funding requested", "Refused -- identifiability risk",
  "Applicant",   "Prior funding success (binary)",                          "Extractable",
  "Applicant",   "Gender",                                                  "In-house only (no dummy data)",
  "Applicant",   "Career stage",                                            "In-house only (no dummy data)",
  "Applicant",   "Scientific productivity",                                 "In-house only -- and unlikely even then",
  "Member",      "Member number / cid",                                    "Extractable",
  "Member",      "Job (reviewer vs. panelist role)",                        "Extractable",
  "Member",      "Self-declared expertise (High/Medium/Low/Not enough)",    "Extractable (can be split into binary columns)",
  "Member",      "Score (final/voted score)",                               "Extractable",
  "Member",      "Initial reviewer scores",                                 "Extractable",
  "Member",      "Past funding success (binary)",                          "Extractable",
  "Member",      "Conflicts of interest",                                  "Extractable (probably -- bundled with expertise data)",
  "Member",      "Gender",                                                  "In-house only (no dummy data)",
  "Member",      "Experience/tenure as a reviewer (distinct from self-declared expertise above; measurement TBD)", "In-house only (no dummy data)"
)

group_starts <- which(!duplicated(cihr_fields$level))
header_rows  <- group_starts + seq_along(group_starts) - 1

cihr_fields |>
  select(indicator, availability) |>
  setNames(c("Indicator", "Availability")) |>
  tt(caption = "CIHR field-by-field data availability, by level") |>
  group_tt(i = setNames(as.list(group_starts), unique(cihr_fields$level))) |>
...
extractable**, and won't appear even in CIHR's own distribution-matched
dummy data. That constraint is why this document exists: the simulation
below is the only rehearsal Aim 2's code gets before it runs once,
unsupervised, on real data.
...
panelists in terms of deviations from the consensus score, given that a deviation occurred:
