...
From the fixed effects we see that we generally recover the simulated parameters. Specifically the 'true' treatment effects in the first column are well approximated by our model. We can also generate the estimated absolute probabilities of deviating and how those are affected by `job` and `expertise` using the `marginaleffects` package
